...
from tkinter.ttk import Notebook, Style, Label, Frame, Button, LabelFrame, Entry, Progressbar, Treeview
...
class file_dialog(_dialog):
	...
	def build_branch(self, branch, entry):
		...
		if entry.is_file():
			...
			else: self.tree.item(branch, tags = ("valid",))
		else: self.tree.item(branch, tags = ("invalid",))
...
class folder_dialog(_dialog):
	...
	def build_branch(self, branch, entry):
		if entry.is_dir(): self.tree.item(branch, tags = ("valid",))
		else: self.tree.item(branch, tags = ("invalid",))
		super().build_branch(branch, entry)
...
def _on_mousewheel(event, widget):
	if platform.system() == "Windows": widget.yview_scroll(-1 * int(event.delta / 120), "units")
	elif platform.system() == "Darwin": widget.yview_scroll(-1 * int(event.delta), "units")
	else: #Linux
		if event.num == 4: widget.yview_scroll(-1, "units")
		elif event.num == 5: widget.yview_scroll(1, "units")

class ScrolledTree(ScrollWrapper, Treeview):
	@_create_frame
